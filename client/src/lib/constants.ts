export const SOCIAL_LINKS = {
  DISCORD: "https://discord.gg/Hu3FxfHnZt",
  INSTAGRAM: "https://www.instagram.com/wlooquantclub/",
  LINKEDIN: "https://linkedin.com/company/waterloo-quant-club",
  EMAIL: "mailto:info@waterlooquantclub.com",
} as const;

export interface MemberInfo {
  name: string;
  role: string;
  program: string;
  bio?: string;
  image?: string;
  linkedin?: string;
  email?: string;
  website?: string;
}

export interface AlumniInfo {
  name: string;
  position: string;
  company: string;
  gradYear: number;
  bio?: string;
  image?: string;
  linkedin?: string;
  email?: string;
  website?: string;
}

export const TEAM: MemberInfo[] = [
  {
    name: "Daniel Shen",
    role: "Co-President",
    program: "Computer Science",
    image: "daniel_shen.jpg",
  },
  {
    name: "Patrick Huang",
    role: "Co-President",
    program: "Software Engineering",
    bio: "30x'd trading marbles in elementary school. In my free time, I like eating food.",
    image: "patrick_huang.jpg",
    linkedin: "https://www.linkedin.com/in/patrick-y-huang/",
    email: "patrick.huang@uwaterloo.ca",
    website: "https://www.patrickhuang.co/",
  },
  {
    name: "Samantha La",
    role: "Co-President",
    program: "Systems Design Engineering",
    bio: "Hi! I'm a Systems Design Engineering student that loves collecting smiskis and reading. Occasionally you can find me at a bank ;)",
    image: "sam_la.png",
    linkedin: "https://www.linkedin.com/in/samanthala/",
    email: "samanthala@waterlooquantclub.com",
  },
  {
    name: "Aurora Shi",
    role: "VP",
    program: "Software Engineering",
    bio: "Artist + SWE who enjoys finding new restaurants, being one with nature, and edm (I love noises). Prev @ Rippling and 2x Bloomberg!",
    image: "aurora_shi.png",
    email: "aurorashi@waterlooquantclub.com",
    website: "https://aurorashi.com/",
  },
  {
    name: "Gordon Wang",
    role: "VP",
    program: "Computing and Financial Management",
    bio: "I'm a CS + Finance student currently interning at Point72. I love soccer, football, and MMA.",
    image: "gordon_wang.jpg",
    linkedin: "https://www.linkedin.com/in/gordon-wang-ab785b237/",
    email: "gordonwang@waterlooquantclub.com",
  },
  {
    name: "Jason Hon",
    role: "VP",
    program: "Computer Science",
    bio: "Computer Science student with a Minecraft YouTube channel. Checkout my website!",
    image: "jason_hon.png",
    linkedin: "https://www.linkedin.com/in/jasonhonhk/",
    email: "jasonhon@waterlooquantclub.com",
    website: "https://jasonhon.com/",
  },
  {
    name: "Marcus Lee",
    role: "VP",
    program: "Mathematics, Economics",
    bio: "I'm a Math and Econ student with an interest in finance. In my free time, I enjoy hiking and eating!",
    image: "marcus_lee.jpg",
    linkedin: "https://www.linkedin.com/in/marcus-lee-4355aa214/",
    email: "mys2lee@uwaterloo.ca",
  },
  {
    name: "Amelie Zhou",
    role: "Executive",
    program: "Mathematics",
    bio: "I like solving puzzles and exploring alternative subcultures. I also sometimes write actuarial exams.",
    image: "amelie_zhou.jpg",
    linkedin: "https://www.linkedin.com/in/amelie-zhou",
    email: "amelieezhou@gmail.com",
  },
  {
    name: "Megan Wu",
    role: "Executive",
    program: "Systems Design Engineering",
    bio: "I'm a Systems Design engineering student with experience in SWE. In my free time, I love travelling, puzzles, and drawing!",
    image: "megan_wu.jpg",
    linkedin: "https://www.linkedin.com/in/megan-bx-wu/",
    email: "meganwu@waterlooquantclub.com",
  },
  {
    name: "Michael Parker",
    role: "Executive",
    program: "Combinatorics & Optimization, Finance (WLU)",
    bio: "3A C&O + Finance (WLU) Double Degree. Usually found on the volleyball court, at Chipotle, or at Fantastic Wok.",
    image: "michael_parker.jpeg",
    linkedin: "https://www.linkedin.com/in/michael-parker-7000a22a1/",
    email: "michaelparker@waterlooquantclub.com",
  },
  {
    name: "Angelina Wu",
    role: "Executive",
    program: "Software Engineering",
  },
  {
    name: "Ian Leung",
    role: "Executive",
    program: "Computing and Financial Management",
    bio: "I'm studying CS + Finance + Stats and I like games, puzzles, and my two cats.",
    image: "ileung.jpg",
    linkedin: "https://www.linkedin.com/in/ianleung12/",
    email: "ianleung@waterlooquantclub.com",
    website: "https://ianleung.com",
  },
  {
    name: "Alex Oláh",
    role: "Executive",
    program: "Masters of Accounting",
    bio: "EY Tax",
    image: "alex_olah.jpg",
    linkedin: "https://www.linkedin.com/in/alexolah",
  },
  {
    name: "Matthew Yang",
    role: "Executive",
    program: "Computing and Financial Management",
    bio: "aspiring renaissance man",
    image: "matthew_yang.jpg",
    linkedin: "https://www.linkedin.com/in/matthew--yang/",
    email: "matthewyang@waterlooquantclub.com",
    website: "https://matthewyang.ca/",
  },
  {
    name: "Kaiya Li-Lin",
    role: "Executive",
    program: "Computing and Financial Management",
    bio: "I love snowboarding, boxing, designing/altering clothes and curating random Spotify playlists.",
    image: "kaiya_li-lin.png",
    linkedin: "https://www.linkedin.com/in/kaiyaemmali77/",
    email: "emmali@waterlooquantclub.com",
  },
  {
    name: "Caroline Lu",
    role: "Executive",
    program: "Data Science",
    bio: "I like reading, math, racket sports, and music.",
    image: "caroline_lu.jpeg",
    linkedin: "https://www.linkedin.com/in/yiyang-lu-caroline",
    email: "carolinelu@waterlooquantclub.com",
  },
  {
    name: "David Kachroo",
    role: "Executive",
    program: "Computing and Financial Management",
    bio: "I'm a CS + Finance (CFM) student in 1B and a Junior Exec for WQC. I enjoy investing, brain teasers (QuantPrep), and coding side projects",
    image: "david_kachroo.jpg",
    linkedin: "https://www.linkedin.com/in/david-kachroo/",
    email: "davidkachroo@waterlooquantclub.com",
  },
  {
    name: "Alfred Zhang",
    role: "Executive",
    program: "Computer Science",
    bio: "I study Computer Science and have a penchant for the unconventional.",
    image: "alfred_zhang.jpg",
    linkedin: "https://www.linkedin.com/in/aswalfredz/",
    email: "alfredzhang@waterlooquantclub.com",
  },
  {
    name: "Elaine Li",
    role: "Executive",
    program: "Computing and Financial Management",
    bio: "On coop term after CFM 2A right now, a fun fact is I'm been on top of a wind turbine",
    image: "elaine_li.jpg",
    linkedin: "https://www.linkedin.com/in/elaine-qy-li",
    email: "elaine.li@waterlooquantclub.com",
  },
];

// TODO: every position/company below is a placeholder — fill in real full-time
// roles before deploying. Images live alongside team photos in @/assets/team/.
export const ALUMNI: AlumniInfo[] = [
  {
    name: "Harry Jiang",
    position: "Quantitative Trader",
    company: "Jane Street",
    gradYear: 2026,
    bio: "2025-2026 President",
    image: "harry_jiang.jpg",
    linkedin: "https://www.linkedin.com/in/harryjiang7/",
    email: "harryjiang@waterlooquantclub.com",
  },
  {
    name: "Ian Zhao",
    position: "Software Engineer",
    company: "Hudson River Trading",
    gradYear: 2026,
    bio: "2025-2026 Vice President",
    image: "ian_zhao.jpg",
    linkedin: "https://www.linkedin.com/in/ian-zhao/",
    email: "ianzhao@waterlooquantclub.com",
  },
  {
    name: "John Huang",
    position: "Quantitative Research Analyst",
    company: "Cubist Systematic Strategies",
    gradYear: 2026,
    bio: "2025-2026 Vice President",
    image: "john_huang.jpg",
  },
  {
    name: "Kenneth Xiao",
    position: "Software Engineer",
    company: "Point72",
    gradYear: 2026,
    bio: "2025-2026 Vice President",
    image: "kenneth_xiao.jpg",
    linkedin: "https://www.linkedin.com/in/kdxiao",
    email: "kdxiao@uwaterloo.ca",
  },
  {
    name: "Soham Parmar",
    position: "Software Engineer",
    company: "Jane Street",
    gradYear: 2026,
    bio: "2025-2026 Executive",
    image: "soham_parmar.jpg",
    linkedin: "https://www.linkedin.com/in/soham-parmar",
    email: "s24parma@uwaterloo.ca",
  },
  {
    name: "Richard Yang",
    position: "Investment Banking",
    company: "RBC Capital Markets",
    gradYear: 2026,
    bio: "2025-2026 Executive",
    image: "richard_yang.jpg",
    linkedin: "https://www.linkedin.com/in/richardyang03/",
    email: "richard.yang1@uwaterloo.ca",
  },
  {
    name: "Moses Xu",
    position: "Quantitative Technologist",
    company: "Radix Trading",
    gradYear: 2026,
    bio: "2025-2026 Executive",
    image: "moses_xu.jpg",
    linkedin: "https://www.linkedin.com/in/moses-xu",
    email: "moses.xu@uwaterloo.ca",
  },
  {
    name: "Joshua Liu",
    position: "Software Engineer",
    company: "Hudson River Trading",
    gradYear: 2026,
    bio: "2025-2026 Executive",
    image: "joshua_liu.jpg",
    linkedin: "https://www.linkedin.com/in/joshua-liu-265681220/",
    email: "joshliu@waterlooquantclub.com",
  },
  {
    name: "Justin Torti",
    position: "Quantitative Trader",
    company: "TransMarket Group",
    gradYear: 2026,
    bio: "2025-2026 Executive",
    image: "justin_torti.png",
    linkedin: "https://www.linkedin.com/in/justin-torti-205146197/",
  },
];

